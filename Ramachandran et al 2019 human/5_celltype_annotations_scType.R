...
gs_folder <- "../../GeneLists/scType DB human"
...
saveRDS(sc@meta.data, file = "processed_Seurat_ob_metadata.rds")
...
for(gs_name in c("List2", "List3")){
  ...
  # Stacked violin plot
  jpeg(sprintf("%s/1_Ramachandran-human_%s_vln.jpeg", out_dir, gs_name), height = 1000, width = 700)
  # pdf(sprintf("%s/1_%s2.pdf", out_dir, gs_name), height=6, width = 4)
  plot2 <- VlnPlot(sc, features = rev(markers),split.by = "Condition", group.by = gs_name, pt.size = 0.1, stack = T,flip = T)  # + ggforce::geom_sina(jitt)
  print(plot2); dev.off()
  
  # UMAP
  jpeg(sprintf("%s/1_Ramachandran-human_%s_umap.jpeg", out_dir, gs_name), height = 500, width = 500)
...
# 
# # Celltype colors
# cts <- unique(sc$Xiong_CellType)
# ct_colors <- RColorBrewer::brewer.pal(length(cts), "Spectral")
# names(ct_colors) <- cts
# DotPlot(sc, features = c("Epcam","Krt19","Cftr","Tspan8","Clu"), group.by = "List3_scType", cols = c("white", "red")) +
#   theme(axis.text.x = element_text(angle=45, hjust=1))
